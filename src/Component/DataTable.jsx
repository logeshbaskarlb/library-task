...
const Table = ({ bookData }) => {
  ...
  const closeAfter15 = (title) =>
    toast(`${title} is deleted`, { type: toast.TYPE.WARNING, autoClose: 2000 });

  const handleDelete = async (bookId, title) => {
    try {
      await axios.delete(
        `https://65615e6adcd355c08323c948.mockapi.io/users/${bookId}`
      );
      closeAfter15(title);
          ...
          <tbody>
            {bookData.books.map((book, index) => (
              <tr key={index}>
                ...
                <td className="text-center ">{book.dop}</td>
